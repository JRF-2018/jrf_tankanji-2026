...
# IME「風」に含まれる単漢字辞書。今回は単漢字の読みごとの頻度などを参
# 考にしている。「風」辞書の読みと漢字は優先割り当てしている。もちろん、
# 「風」辞書と同じ場所にではない。
  ...
  while (<DIC>) {
	...
	if ($& =~ /^$TRUEKANJI$/) {
	  ...
	  if ($tankanji eq $nookuri) {
		...
		$YOMI_HINDO{$tankanji} = {} if ! exists $YOMI_HINDO{$tankanji};
		$YOMI_HINDO{$tankanji}->{$yomi} = []
		  if ! exists $YOMI_HINDO{$tankanji}->{$yomi};
		push(@{$YOMI_HINDO{$tankanji}->{$yomi}}, $hindo);
	      }
	    }
	  }
	}
