...
# IME「風」に含まれる単漢字辞書。今回は単漢字の読みごとの頻度としての
# 順序を参考にしている。
...
our $JISX0213_C = 
  "(?:\x8f[\xa1-\xfe][\xa1-\xfe]|\xf4[\xa7-\xfe]|[\xf5-\fe][\xa1-\xfe]"
  ...
  . "|\xcf[\xd5-\xfd])";
our $IN_MS = "(?:\xad[\a1-\xd6\xdf-\xef\xf3\xf8\xf9])";
...
sub complement_itaiji {
  ...
  foreach my $k (sort keys %ITAIJI) {
	...
	if (exists $tbl->{$i}) {
	  $new = $tbl->{$i} if ! defined $new || %{$new} > %{$tbl->{$i}};
	}
      }
...
sub compare_tankanji_hindo {
  ...
  if ($r == 0) {
    ...
    } elsif (exists $BUSHU{$b}) {
      $r = 1;
    }
  }
  if ($r == 0) {
    if (substr($a, 0, 1) ne "\x8f" && substr($b, 0, 1) ne "\x8f") {
      $r = $b cmp $a;
    } elsif (substr($a, 0, 1) ne "\x8f") {
      $r = 1;
    } elsif (substr($b, 0, 1) ne "\x8f") {
      $r = -1;
    } else {
      $r = $b cmp $a;
    }
  ...
  foreach my $tankanji (sort {&compare_tankanji_hindo($b,$a)} (keys %{$tbl})) {
	...
	foreach my $yomi (keys %{$tbl->{$tankanji}}) {
	  ...
	  for (my $page = 0; ; $page++) {
		...
		&& defined $YHAIRETSU{$yomi}->[$page * 40 + $pos] ne $tankanji) {
	      next;
	    }
	...
	return $r;
      ...
      my @a = sort {pos_score($pospref{$a}) <=> pos_score($pospref{$b})} (keys %pospref);
      my $truepos = $a[0];
...
sub swap_yhairetsu_by_yomi_hindo {
  ...
  foreach my $yomi (keys %YHAIRETSU) {
	...
	push(@a, $YHAIRETSU{$yomi}->[$page * 40 + $pos]);
      ...
      sub score_k {
      	my ($yomi, $kanji, $ytok) = @_;
      	return 40 if ! exists $ytok->{$yomi};
       	my @l = @{$ytok->{$yomi}};
      ...
      my @b = sort {score_k($yomi, $a, \%ytok) <=> score_k($yomi, $b, \%ytok)} @a;
      for (my $i = 0; $i < @a; $i++) {
	if ($a[$i] ne $b[$i]) {
	  #warn "swap $a[$i] $b[$i]\n";
	  last;
